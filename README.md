...
**Status:** 🟢 active · 🟡 dormant (works, but quiet) · 🔴 archived (read-only). Everything listed is usable today - we don't list announced-but-unreleased projects.
...
- 🟢 [OP-Z SysEx](https://github.com/kmorrill/op-z-sysex) - Adds device-proven reads and guarded writes for projects, patterns, MIDI settings, and files beyond the earlier parsers and format documentation.
...
- 🟡 [Xfer Records OP-1 Drum Utility](https://rekkerd.org/xfer-records-releases-op-1-drum-utility) - Free Win/Mac plugin merging 24 one-shot samples into a valid OP-1 drumkit AIF file.
